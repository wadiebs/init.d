...
# Improved: error handling, tool validation, URL verification, architecture detection
###
#
# About:
...
#   It will also configure the duckstation AppImage as the default emulator for PSX ROMs in ES-DE.
#
# Guide:
...
if ! __latest_package_json=$(wget -O - -o /dev/null https://api.github.com/repos/stenzek/duckstation/releases/latest 2>/dev/null); then
    echo "ERROR: Failed to fetch release information from GitHub" >&2
    exit 1
fi

if [ -z "${__latest_package_json}" ]; then
    echo "ERROR: Failed to fetch release information from GitHub" >&2
    exit 1
fi
...
case ${__arch} in
    x86_64)
        __appimage_pattern="DuckStation-x64.AppImage"
        ;;
    aarch64)
        __appimage_pattern="DuckStation-arm64.AppImage"
        ;;
    armv7l)
        __appimage_pattern="DuckStation-armhf.AppImage"
        ;;
    *)
        __appimage_pattern="DuckStation-x64.AppImage"
        ;;
...
__latest_package_version=$(echo ${__latest_package_json:?} | jq -r '.tag_name')
__latest_package_url=$(echo ${__latest_package_json:?} | jq -r '.assets[] | select(.name | endswith("'"${__appimage_pattern}"'")) | .browser_download_url' | head -1)

# Validate that we got a valid download URL
if [ -z "${__latest_package_url}" ]; then
    echo "ERROR: Could not find a compatible AppImage for architecture ${__arch}" >&2
    exit 1
fi

__installed_version=$(catalog -g ${package_name,,})
print_step_header "Latest ${package_name:?} version: ${__latest_package_version:?} (installed: ${__installed_version:-None})"
...
if ([ ! -f "${package_executable:?}" ] || [ "${__installed_version:-X}" != "${__latest_package_version:?}" ]); then
    ...
    print_step_header "Latest version of ${package_name:?} already installed"
fi
...
mkdir -p \
    ...
    "${__emulation_path:?}"/storage/duckstation/{states,memcards,bios,screenshots,covers,cache,cheats,dump,gamesettings,inputprofiles,shaders,textures}
